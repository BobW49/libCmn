...
''' Build the Miscellaneous Classes and Objects
'''
...
def         parse_args(listArgV=None):
    ...
    cmd_prs.add_argument('-l', '--lex', action='store_false', dest='AsmLex',
                         default=True, help='Do not generate LexAsm'
                         )
    cmd_prs.add_argument('-p', '--prs', action='store_true', dest='AsmPrs',
                         default=False, help='Generate AsmPrs'
                         )
...
def         perform_actions():
    ...
    if args.verbose > 0:
        print('*****************************************')
        print('*       Building the Classes            *')
        print('*****************************************')
        print()

    print('Building the GenItem_Types.')
    rc = do_cmd("genEnum --prefix=GenItem --name=Type --files genitem_types.txt")
    if rc != 0:
        print('ERROR - genEnum GenItem_Types failed!')
        return 4
    rc = do_cmd("cp GenItem_Type_routines.c ../src/")
    if rc != 0:
        print('ERROR - genEnum GenItem_Types cp failed!')
        return 4
    rc = do_cmd("cp GenItem_Type_tables.c   ../src/")
    if rc != 0:
        print('ERROR - genEnum GenItem_Types cp failed!')
        return 4
    rc = do_cmd("cp GenItem_Type.h          ../src/")
    if rc != 0:
        print('ERROR - genEnum GenItem_Types cp failed!')
        return 4


    print('Building the PropItem_Types.')
    rc = do_cmd("genEnum --prefix=PropItem --name=Type --files propitem_types.txt")
    if rc != 0:
        print('ERROR - genEnum PropItem_Types failed!')
        return 4
    rc = do_cmd("cp PropItem_Type_routines.c ../src/")
    if rc != 0:
        print('ERROR - genEnum PropItem_Types cp failed!')
        return 4
    rc = do_cmd("cp PropItem_Type_tables.c   ../src/")
    if rc != 0:
        print('ERROR - genEnum PropItem_Types cp failed!')
        return 4
    rc = do_cmd("cp PropItem_Type.h          ../src/")
    if rc != 0:
        print('ERROR - genEnum PropItem_Types cp failed!')
        return 4

    print('Building the PropItem_Vis.')
    rc = do_cmd("genEnum --prefix=PropItem --name=Vis --files propitem_vis.txt")
    if rc != 0:
        print('ERROR - genEnum PropItem_Vis failed!')
        return 4
    rc = do_cmd("cp PropItem_Vis_routines.c ../src/")
    if rc != 0:
        print('ERROR - genEnum PropItem_Vis cp failed!')
        return 4
    rc = do_cmd("cp PropItem_Vis_tables.c   ../src/")
    if rc != 0:
        print('ERROR - genEnum PropItem_Vis cp failed!')
        return 4
    rc = do_cmd("cp PropItem_Vis.h          ../src/")
    if rc != 0:
        print('ERROR - genEnum PropItem_Vis cp failed!')
        return 4
    ...
    """
    if args.AsmLex:
        print('Building AsmLex.re')
        rc = do_cmd("./mkLex.sh")
        if rc != 0:
            print('ERROR - AsmLex.re failed!')
            return 4

    if args.AsmPrs:
        print('Building AsmPrs.y')
        rc = do_cmd("lemon -l -u AsmPrs.y")
        if rc != 0:
            print('ERROR - AsmPrs.y failed!')
            return 4
        rc = do_cmd("cp AsmPrs.c          ../src/")
        if rc != 0:
            print('ERROR - AsmPrs.y failed!')
            return 4
        rc = do_cmd("cp AsmPrs.h          ../src/")
        if rc != 0:
            print('ERROR - AsmPrs.y failed!')
            return 4
    """
