...
def         parse_args(listArgV=None):
    ...
    cmd_prs.add_argument('-a', '--arch', action='store', dest='arch',
    ...
    cmd_prs.add_argument('--final', action='store_true', dest='final',
    ...
    cmd_prs.add_argument('--misc', action='store_true', dest='misc',
    ...
    cmd_prs.add_argument('-t', '--test', action='store_true', dest='test',
    ...
    if args.arch == 'CPU_ARCH':
        args.arch = os.environ.get('CPU_ARCH')
...
def         perform_actions():
    ...
    '''
    global      args
    make_file_name = f"Makefile_{args.ostype}_{args.arch}.txt"
    ...
    if args.make:
       ...
       if not os.path.exists(make_file_name):
           print('ERROR - genMake failed!')
           return 4
       rc = do_cmd("cp %s Makefile" % make_file_name)
       if rc != 0:
           print('ERROR - cp failed!')
           return 4

    # Build the Miscellaneous Enums, Lexers, Parsers, etc if present.
    if args.misc and os.path.exists("./mkMisc.py"):
        rc = do_cmd("./mkMisc.py")
        if rc != 0:
            print('ERROR - mkMisc.py failed!')
            return 4
    if args.misc and os.path.exists("./mkMisc.sh"):
        rc = do_cmd("./mkMisc.sh")
        if rc != 0:
            print('ERROR - mkMisc.sh failed!')
            return 4

    # Build the program.
    rc = do_cmd(f"make -f {make_file_name} clean")
    if rc != 0:
        print('ERROR - make clean failed!')
        return 4
    rc = do_cmd(f"make -f {make_file_name} build")
    if rc != 0:
    ...
    # Test the program.
    if args.test:
        rc = do_cmd(f"make -f {make_file_name} check")
        if rc != 0:
    ...
    # Install the program.
    if args.install:
        rc = do_cmd(f"make -f {make_file_name} install")
        if rc != 0:
            print('ERROR - make install failed!')
            return 4

    # Perform any finalization if needed.
    if args.final and os.path.exists("./mkFinal.py"):
        rc = do_cmd("./mkFinal.py")
        if rc != 0:
            print('ERROR - mkFinal.py failed!')
            return 4
    if args.final and os.path.exists("./mkFinal.sh"):
        rc = do_cmd("./mkFinal.sh")
        if rc != 0:
