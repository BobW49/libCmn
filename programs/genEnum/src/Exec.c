...
extern "C" {
    ...
    ****************************************************************/
    ...
    ASTRARRAY_DATA * Exec_getComments (
        EXEC_DATA       *this
    )
    {
    ...
    )
    {
        ...
        obj_Retain(pValue);
        if (this->pComments) {
            obj_Release(this->pComments);
        }
        this->pComments = pValue;
...
#endif
        this->pSuffixA = pSuffixA;
        if (pInputPath) {
            eRc = Path_IsFile(pInputPath);
            if (ERESULT_FAILED(eRc)) {
                return eRc;
            }
            pIn = TextIns_NewFromPath(pInputPath, 4);
            if (OBJ_NIL == pIn) {
                return ERESULT_FILE_NOT_FOUND;
            }
        }

        pArray = Exec_ReadFile(this, pIn, pLibsA);
        obj_Release(pIn);
        pIn = OBJ_NIL;
        if (OBJ_NIL == pArray) {
    ...
    )
    {
        ...
        PATH_DATA       *pOutPath = OBJ_NIL;
        char            fileName[256];

        // Do initialization.
        TRC_OBJ(this,"%s: %s\n", __func__, Path_getData(pInputPath));
#ifdef NDEBUG
#else
        if (!Exec_Validate(this)) {
            DEBUG_BREAK();
        ...
        eRc = Path_Split(pInputPath, &pDrive, &pDir, &pFileName, &pExt);
        if (this->fFiles) {
            strcpy(fileName,pPrefixA);
            strcat(fileName,"_");
            strcat(fileName,pNameA);
            pOutPath =  Path_NewFromComponentsA(
                                                pDrive ? AStr_getData(pDrive) : NULL,
                                                pDir ? AStr_getData(pDir) : NULL,
                                                fileName,
                                                ".h"
                        );
            if (pOutPath) {
                eRc = Path_Clean(pOutPath, NULL, OBJ_NIL);
                pOut = fopen(Path_getData(pOutPath), "w");
                if (pOut == NULL) {
                    pOut = stdout;
                }
            }
        ...
        if (AStrArray_getSize(this->pComments)) {
            fprintf(pOut, "\t/*\n");
            for (i=0; i<AStrArray_getSize(this->pComments); i++) {
                ASTR_DATA           *pCmnt = AStrArray_Get(this->pComments, i+1);
                fprintf(pOut, "\t %s\n", AStr_getData(pCmnt));
            }
            fprintf(pOut, "\t */\n\n");
        }
        fprintf(pOut, "\t/* The following enum was generated from:\n");
        fprintf(pOut, "\t * \"%s.%s\"\n", AStr_getData(pFileName), AStr_getData(pExt));
        ...
        for (i=0; i<iMax; i++) {
            ...
            if (pItem) {
                ...
                if (pWrk) {
                    if (pDescA) {
                        int     fillLen = 53 - AStr_getLength(pWrk);
                        if (fillLen > 0) {
                            AStr_AppendCharRepeatA(pWrk, fillLen, ' ');
                        }
                        fprintf(pOut, "%s// %s\n", AStr_getData(pWrk), pDescA);
                    } else {
                        fprintf(pOut, "%s\n", AStr_getData(pWrk));
                    }
                    obj_Release(pWrk);
    ...
    )
    {
        ...
        if (this->pStart) {
            fprintf(
                    pOut,
                    "\t\tif ((value - c%s_%s_offset) >= c%s_%s_index) {\n",
                    pPrefixA,
                    pNameA,
                    pPrefixA,
                    pNameA
        ...
        if (this->pStart) {
            fprintf(
                    pOut,
                    "\t\tif (%s_%s_index[value - c%s_%s_offset]) {\n",
                    pPrefixA,
                    pNameA,
                    pPrefixA,
                    pNameA
            ...
            fprintf(
                    pOut,
                    "\t\t\treturn %s_%s_entries[%s_%s_index[value - c%s_%s_offset] - 1].pEnum;\n",
                    pPrefixA,
                    pNameA,
                    pPrefixA,
                    pNameA,
        ...
        if (this->pStart) {
            ...
            fprintf(
                    pOut,
                    "\t\t\treturn %s_%s_entries[%s_%s_index[value - c%s_%s_offset] - 1].pName;\n",
                    pPrefixA,
                    pNameA,
                    pPrefixA,
                    pNameA,
        ...
        if (this->fXlate) {
            fprintf(pOut, "\t// Given an enum value, return its translation value.\n");
            fprintf(pOut, "\tuint32_t\t\t\t%s_%sToXlate (\n", pPrefixA, pNameA);
            fprintf(pOut, "\t\tuint32_t\t\tvalue\n");
            fprintf(pOut, "\t)\n");
            fprintf(pOut, "\t{\n");
            if (this->pStart) {
            ...
            if (this->pStart) {
                ...
                fprintf(
                        pOut,
                        "\t\t\treturn %s_%s_entries[%s_%s_index[value - c%s_%s_offset] - 1].xlate;\n",
                        pPrefixA,
                        pNameA,
                        pPrefixA,
                        pNameA,
                ...
                fprintf(
                        pOut,
                        "\t\t\treturn %s_%s_entries[%s_%s_index[value] - 1].xlate;\n",
                        pPrefixA,
                        pNameA,
                        pPrefixA,
                        pNameA
        ...
        for (i=0; i<iMax; i++) {
            ...
            if (pItem) {
                if (this->pStart) {
                    ...
                    if (this->fXlate) {
                        if (Item_getXlate(pItem)) {
                            fprintf(pOut, ", %s", AStr_getData(Item_getXlate(pItem)));
                        } else {
                            fprintf(pOut, ", 0");
                        }
                    }
    ...
    )
    {
        ...
        this->pComments = AStrArray_New( );
        if (OBJ_NIL == this->pComments) {
            DEBUG_BREAK();
            obj_Release(this);
            return OBJ_NIL;
        }
...
//#if defined(__APPLE__)
    ...
    )
    {
        ...
        for (;;) {
            eRc = TextIns_GetLineAStr(pIn, &pLine, &loc);
            if (ERESULT_FAILED(eRc))
                break;
            if (pLine) {
                AStr_Trim(pLine);
                ...
                if (AStr_getLength(pLine) && (AStr_CharGetFirstW32(pLine) == '#')) {
                    // Comment Line that we skip.
                    obj_Release(pLine);
                    pLine = OBJ_NIL;
                    continue;
                }
                if (AStr_getLength(pLine) && (AStr_CharGetFirstW32(pLine) == '*')) {
                    // Comment Line passed through to .h file
                    AStrArray_AppendStr(this->pComments, pLine, NULL);
                    obj_Release(pLine);
                    pLine = OBJ_NIL;
                    continue;
                }
                if (AStr_getLength(pLine) && (AStr_CharGetFirstW32(pLine) == '{')) {
                    uint32_t     end = 0;
                    // Found include file!
                    AStr_CharFindNextW32(pLine, &end, '}');
                    if (end && (end > 2)) {
                        ASTR_DATA       *pStr = OBJ_NIL;
                        PATH_DATA       *pPath;
                        fprintf(stderr, "line len: %d\n", AStr_getLength(pLine));
                        pStr = AStr_NewFromMidA(AStr_getData(pLine), 2, (end - 2));
                        if (pLibsA) {
                            AStr_PrependA(pStr, pLibsA);
                        }
                        ...
                        if (pPath) {
                            ...
                            eRc = TextIns_NewSrcFromPath(pIn, pPath, 4);
                            obj_Release(pPath);
                            pPath = OBJ_NIL;
                        }
                    }
                ...
                if (AStr_getLength(pLine) && (AStr_CharGetFirstW32(pLine) == '=')) {
                    // Increase "lineNo" (ie enumeration number) without defining items.
                    uint32_t            newLineNo;
                    AStr_Remove(pLine, 1, 1);
                    newLineNo = (uint32_t)AStr_ToUint64(pLine);
                    if (newLineNo > lineNo)
                        lineNo = newLineNo;
                    obj_Release(pLine);
                    pLine = OBJ_NIL;
                ...
                if (AStr_getLength(pLine) > 0) {
                    ...
                    if (this->pSuffixA) {
                        AStr_AppendA(pStr, this->pSuffixA);
                    }
                    ...
                    if (Scanner_MatchChrW32(pScan, ',')) {
                        ...
                        if (Scanner_MatchChrW32(pScan, ',')) {
                            ...
                            if (Scanner_MatchChrW32(pScan, ',')) {
                                pXlate = Scanner_ScanStringToAStr(pScan);
                                if (pXlate && (AStr_getLength(pXlate) > 0)) {
                                    Item_setXlate(pItem, pXlate);
                                    this->fXlate = 1;
                                }
                                obj_Release(pXlate);
                                pXlate = OBJ_NIL;
