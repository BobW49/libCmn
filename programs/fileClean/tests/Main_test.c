...
#ifdef   MAIN_MSGS
        Main_setMsg(pObj, (void *)Test_MsgInfo, (void *)Test_MsgWarn, this);
#endif
...
)
{
    ...
    if (pObj1) {
        ...
        TestForTrue(fRc, "Failed Ident Test");
#ifdef   MAIN_MSGS
        Main_setMsg(pObj1, (void *)Test_MsgInfo, (void *)Test_MsgWarn, this);
#endif
...
)
{
    ...
    const
    char            *pPathInA = TEST_FILES_DIR "Test1.txt";
    const
    char            *pPathWorkA = TEST_FILES_DIR "Test1.work.txt";
    const
    char            *pResultPathA = TEST_FILES_DIR "Test1_Result.txt";
    PATH_DATA       *pPathResult = OBJ_NIL;
    ...
    if (pObj) {
        ...
        eRc = File_CopyFileA(pPathInA, pPathWorkA);
        TestForSuccess("");
        pPath = Path_NewA(pPathWorkA);
        ...
        pPathResult = Path_NewA(pResultPathA);
        TestForNotNull(pPathResult, "");
        eRc = Path_Clean(pPathResult, NULL, OBJ_NIL);
        TestForSuccess("");
        pResult = AStr_NewFromUtf8File(pPathResult);
        TestForNotNull(pResult, "");

        iRc = str_CompareSpcl(AStr_getData(pStr), AStr_getData(pResult), &offset);
        if (iRc) {
            fprintf(stderr, "\tERROR: iRc=%d  offset=%04X\n", iRc, offset);
        }
        TestForTrue((0 == iRc), "");
        obj_Release(pStr);
        pStr = OBJ_NIL;

        TRC_OBJ(pObj, "\t\tDeleting: %s\n", Path_getData(pPath));
...
)
{
    ...
    //TestExec("Copy01", Test_Main_Copy01, NULL, NULL);
