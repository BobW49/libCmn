...
extern "C" {
    ...
    )
    {
        ...
        if (ERESULT_OK(eRc)) {
            ...
            if (pWrk) {
                NodeTree_setErrorStr(pObject, pWrk);
                obj_Release(pWrk);
            }
    ...
    )
    {
        ...
        eRc =  NodeTree_ParseJsonFields(pParser, pObject);
        if (ERESULT_FAILED(eRc)) {
            obj_Release(pObject);
            pObject = OBJ_NIL;
        }
    ...
    )
    {
        ...
        if (pStr) {
            ...
            eRc = NodeTree_ToJsonFields(this, pStr);      
    ...
    )
    {
        ...
        /*  Create so that the nodes are output with their siblings in an array.
            Use NodeTree_VisitPostOrder() to recurse through the tree or just
            follow the links with a separate method from this one.
            Each node would have:
                "node:" with node data
                "root:" true/false
                "siblings:" array of nodes
         */
        JsonOut_Append_u32("size", this->size, pStr);
        // JsonOut_Append_Object("open", this->pOpen, pStr);
        // JsonOut_Append_Object("close", this->pClose, pStr);
