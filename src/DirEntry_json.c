...
extern "C" {
    ...
    )
    {
        ...
        if (!ERESULT_FAILED(eRc)) {
            pObject->pCreationTime = RW_DateTime_ParseJsonObject(pParser);
            JsonIn_SubObjectEnd(pParser);
        }
        eRc = JsonIn_SubObjectInHash(pParser, "ModifiedTime");
        if (!ERESULT_FAILED(eRc)) {
            pObject->pModifiedTime = RW_DateTime_ParseJsonObject(pParser);
            JsonIn_SubObjectEnd(pParser);
        }
        eRc = JsonIn_SubObjectInHash(pParser, "StatusChangeTime");
        if (!ERESULT_FAILED(eRc)) {
            pObject->pStatusChangeTime = RW_DateTime_ParseJsonObject(pParser);
            JsonIn_SubObjectEnd(pParser);
        }
