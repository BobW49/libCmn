...
extern "C" {
    ...
    )
    {
        ...
        if (ERESULT_OK(eRc)) {
            ...
            if (pWrk) {
                PathArray_setErrorStr(pObject, pWrk);
                obj_Release(pWrk);
            }
            JsonIn_SubObjectEnd(pParser);
        }
    ...
    {
        ERESULT         eRc;
        PATHARRAY_DATA   *pObject = OBJ_NIL;
        const
        OBJ_INFO        *pInfo;
        //int64_t         intIn;
        //ASTR_DATA       *pWrk;
        ...
        pInfo = obj_getInfo(PathArray_Class());
        ...
        pObject = PathArray_New( );
        if (OBJ_NIL == pObject) {
            goto exit00;
        }
        
        eRc =  PathArray_ParseJsonFields(pParser, pObject);
        if (ERESULT_FAILED(eRc)) {
            obj_Release(pObject);
            pObject = OBJ_NIL;
        }
    ...
    )
    {
        ...
        pObject = PathArray_ParseJsonObject(pParser);
    ...
    )
    {
        ...
        if (pStringA) {
            ...
            pObject = PathArray_NewFromJsonString(pStr);
            obj_Release(pStr);
            pStr = OBJ_NIL;
        }
    ...
    )
    {
        ...
        if (pStr) {
            ...
            eRc = PathArray_ToJsonFields(this, pStr);      
